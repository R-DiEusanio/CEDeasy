...
import { useCreateClientPost } from '../src/lib/queries'
...
interface CreateClientPostSheetProps {
  sheetRef: React.RefObject<BottomSheetModal>
}

// Creazione post lato Cliente (Consulenza): niente "Note interne" (campo privato
// dello SMM) e niente scelta di brand — il post nasce sempre CLIENT_DRAFT sul
// proprio brand, derivato server-side (vedi createClientPost in supabase/posts.ts).
export function CreateClientPostSheet({ sheetRef }: CreateClientPostSheetProps) {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      Toast.show({ type: 'success', text1: 'Bozza creata!' })
      reset()
      setDate(new Date())
  ...
  return (
    <Sheet ref={sheetRef} title="Nuovo post" snapPoints={['92%']} scrollable>
