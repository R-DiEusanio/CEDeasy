    ...
    <div className="rounded-2xl border border-border bg-card p-3 shadow-[var(--shadow-soft)] sm:p-5">
      ...
      <div className="grid grid-cols-7 gap-1 sm:gap-2">
        {cells.map((day, i) => {
          ...
          // CORREZIONE: Generiamo una stringa YYYY-MM-DD locale per evitare slittamenti UTC
          const localDateStr = day 
              ...
              {day && (
                <>
                  <div className="mb-1 flex items-center justify-between">
                    <span className={cn("text-[11px] font-semibold sm:text-xs", isToday ? "text-primary" : "text-muted-foreground")}>
                      {day}
                    </span>
                    <button
                      onClick={() => onCreateForDate(localDateStr)}
